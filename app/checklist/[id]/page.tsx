...
export default function ChecklistPage() {
  ...
  useEffect(() => {
    ensureUser().then(user => setUid(user.uid))
    ...
    if (isNovo) {
      ...
      if (rascunho) {
        ...
        if (rascunho.respostas) {
          ...
          rascunho.respostas.forEach((r: RespostaItem) => {
            map[r.itemId] = r.conforme
          })
          setRespostas(map)
        }
      }
    } else {
      getRegistroPorId(id).then(reg => {
        if (!reg) return
        setRegistro(reg)
        setUid(reg.uid)
  ...
  function buildRegistro(status: RegistroChecklist['status']): RegistroChecklist {
    return {
      id: registro?.id ?? '',
      policyId: registro?.policyId ?? gerarId(),
      uid: registro?.uid ?? uid,
  ...
  const criticos = CATS.flatMap(c => c.items.filter(i => i.criticidade === 'Alta')).filter(i => !respostas[i.id]).length
  ...
  return (
    <main style={{ maxWidth: 820, margin: '0 auto', padding: '40px 24px 80px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 32 }}>
        ...
        <div>
          <h1 style={{ fontSize: 18, fontWeight: 600 }}>{isNovo ? 'Novo checklist' : `${MESES[parseInt(mes) - 1]} ${ano} · v${registro?.version ?? 1}`}</h1>
          <p style={{ fontSize: 13, color: 'var(--text-muted)' }}>{isNovo ? 'Preencha os itens abaixo e envie para aprovação.' : `Preenchido por ${analista}`}</p>
        </div>
      </div>

      {isNovo && (
        <div className="card" style={{ padding: '12px 16px', marginBottom: 16, fontSize: 12, color: 'var(--text-muted)' }}>
          Usuário autenticado (Auth): <strong>{uid || 'carregando...'}</strong>
        </div>
      )}
        ...
        <div className="card" style={{ padding: '20px 24px', marginBottom: 24 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
            ...
            <div>
              ...
              <select value={mes} onChange={e => setMes(e.target.value)} style={{ width: '100%', padding: '8px 12px', borderRadius: 'var(--radius)', background: 'var(--bg-subtle)', border: '1px solid var(--border-md)', color: 'var(--text)', fontFamily: 'var(--font-sans)', fontSize: 13, outline: 'none' }}>
                {MESES.map((m, i) => (
            ...
            <div>
              ...
              <input type="number" value={ano} onChange={e => setAno(Number(e.target.value))} style={{ width: '100%', padding: '8px 12px', borderRadius: 'var(--radius)', background: 'var(--bg-subtle)', border: '1px solid var(--border-md)', color: 'var(--text)', fontFamily: 'var(--font-sans)', fontSize: 13, outline: 'none' }} />
            </div>
          </div>
        </div>
      )}
        ...
        {CATS.map(cat => {
          ...
          return (
            <div key={cat.id} className="card" style={{ overflow: 'hidden', borderColor: aberta ? cor : 'var(--border)' }}>
              <button onClick={() => setCatAberta(aberta ? '' : cat.id)} style={{ width: '100%', padding: '16px 20px', background: 'none', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'space-between', color: 'var(--text)' }}>
              ...
              {aberta && (
                <div style={{ borderTop: '1px solid var(--border)', padding: '8px 20px 16px' }}>
                  {cat.items.map(item => {
                    ...
                    return (
                      <div key={item.id} onClick={() => toggle(item.id)} style={{ display: 'flex', alignItems: 'flex-start', gap: 12, padding: '10px 0', borderBottom: '1px solid var(--border)', cursor: soLeitura ? 'default' : 'pointer', opacity: soLeitura ? 0.8 : 1 }}>
                        <div style={{ width: 18, height: 18, borderRadius: 4, flexShrink: 0, marginTop: 1, border: `1.5px solid ${checked ? cor : 'var(--border-md)'}`, background: checked ? corDim : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.15s' }}>
                          {checked && (
                        ...
                        <span style={{ fontSize: 13, flex: 1, lineHeight: 1.5, color: checked ? 'var(--text-muted)' : 'var(--text)', textDecoration: checked ? 'line-through' : 'none', transition: 'all 0.15s' }}>{item.texto}</span>
                        <span className={`badge badge-${item.criticidade.toLowerCase().replace('é', 'e')}`}>{item.criticidade}</span>
                      </div>
                    )
                  })}
                </div>
      ...
      {!soLeitura && (
        <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end', flexWrap: 'wrap' }}>
          <button className="btn" onClick={baixarPDF} disabled={gerandoPDF}>{gerandoPDF ? 'Gerando PDF...' : 'Baixar PDF'}</button>
          <button className="btn" onClick={() => salvar('rascunho')} disabled={salvando}>{salvando ? 'Salvando...' : 'Salvar rascunho'}</button>
          <button className="btn-primary btn" onClick={() => salvar('enviado')} disabled={salvando}>{salvando ? 'Enviando...' : 'Enviar para aprovação'}</button>
        </div>
      )}
    </main>
  )
