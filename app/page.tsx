    ...
    <main style={{ maxWidth: 820, margin: '0 auto', padding: '48px 24px' }}>
      ...
      <div style={{ marginBottom: 40 }}>
        ...
        <p style={{ fontSize: 14, color: 'var(--text-muted)' }}>
          Gestão mensal de políticas de segurança da informação · ISO/IEC 27001
        </p>
      </div>
      ...
      {ultimo && pctUltimo !== null ? (
        <div className="card fade-up" style={{ padding: '24px 28px', marginBottom: 24, borderColor: 'var(--border-md)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 16 }}>
            <div>
              ...
              <p style={{ fontSize: 20, fontWeight: 600 }}>
                {MESES[parseInt(ultimo.mes) - 1]} {ultimo.ano}
              </p>
              <p style={{ fontSize: 13, color: 'var(--text-muted)', marginTop: 4 }}>
                Analista: {ultimo.analista} · {new Date(ultimo.dataPreenchimento).toLocaleDateString('pt-BR')}
              </p>
            </div>
            <div style={{ textAlign: 'right' }}>
              <div style={{
                fontSize: 40, fontWeight: 600,
                color: pctUltimo >= 70 ? 'var(--green)' : pctUltimo >= 40 ? 'var(--amber)' : 'var(--red)'
              }}>
                {pctUltimo}%
              </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{
              ...
              color: STATUS_COR[ultimo.status] ?? 'var(--text-muted)'
            }}>
              ● {STATUS_LABEL[ultimo.status]}
            </span>
        ...
        <div className="card fade-up" style={{ padding: '32px 28px', marginBottom: 24, textAlign: 'center' }}>
          <p style={{ fontSize: 14, color: 'var(--text-muted)', marginBottom: 16 }}>
            Nenhum checklist preenchido ainda. Comece agora.
          </p>
        </div>
      )}
      ...
      {registros.length > 0 && (
        <div>
          <p style={{ fontSize: 13, fontWeight: 500, color: 'var(--text-muted)', marginBottom: 12, textTransform: 'uppercase', letterSpacing: '0.06em' }}>
            Histórico
          </p>
                ...
                <Link key={r.id} href={`/checklist/${r.id}`} style={{ textDecoration: 'none' }}>
                  ...
                  onMouseLeave={e => (e.currentTarget.style.borderColor = 'var(--border)')}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                      <span style={{
                        fontSize: 12, fontWeight: 500,
                        color: STATUS_COR[r.status] ?? 'var(--text-muted)'
                      }}>
                        {STATUS_LABEL[r.status]}
                      </span>
                      <span style={{
                        fontSize: 15, fontWeight: 600,
                        color: pct >= 70 ? 'var(--green)' : pct >= 40 ? 'var(--amber)' : 'var(--red)'
                      }}>
                        {pct}%
                      </span>
                    </div>
